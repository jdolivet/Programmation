...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            if (SP != 2 || guillemet != 0 || part1 == NULL || part2 == NULL || part3 == NULL)
            {
                error(400);
                continue;
            }
            ...
            // Error 405
            if (strcmp(part1, "GET") != 0)
            {
                error(405);
                continue;
            }
            // Error 505
            if (strcmp(part3, "HTTP/1.1") != 0)
            {
                error(505);
                continue;
            } 
            // Error 501
            if (ext != 1 || (int) part2[0] != 47)
            {
                error(501);
                continue;
            }
            ...
            char *requete = strchr(part2, 63);
            int adjust_path;
            int longueur = (requete == NULL) ? 1 : strlen(requete);
            char query[longueur];
            if (requete == NULL)
            {
                adjust_path = 0;
            ...
            {
                adjust_path = 1;
                strncpy(query, requete + 1, longueur - 1);
                query[longueur] = '\0';
            }
            
            // concatenate root and absolute-path           
            int len_path = strlen(part2) - longueur + 2 - adjust_path;          
            char absolute_path[len_path];
            strncpy(absolute_path, part2, len_path - 1);           
            int len_root = strlen(root);
            char path[len_path + len_root + 1];
            strcpy(path, root);
            strcat(path, absolute_path);
            path[len_path + len_root + 1] = '\0';  
            ...
            char *ext_prev = strchr(path, '.');
            int len_ext = strlen(ext_prev);        
            char extension[len_ext];
            strncpy(extension, ext_prev + 1, len_ext);
...
const char* lookup(const char* extension)
{
    if (extension != NULL)
    {
        if ( strcmp(extension, "css") == 0 || strcmp(extension, "CSS") == 0)
        {
            return "text/css";
        }
        ...
        if ( strcmp(extension, "ico") == 0 || strcmp(extension, "ICO") == 0)
        {
            return "image/x-icon";
        } 
        ...
        if ( strcmp(extension, "js") == 0 || strcmp(extension, "JS") == 0)
        {
