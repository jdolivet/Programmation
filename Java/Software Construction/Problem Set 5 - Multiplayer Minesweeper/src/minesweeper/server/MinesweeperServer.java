...
public class MinesweeperServer {
    ...
    /** Default square board size. */
    private static final String HELP_MESSAGE = "COMMANDS: look | dig x y | flag x y | deflag x y | help | bye";
    
    /** Socket for receiving incoming connections. */
    private final ServerSocket serverSocket;
    /** True if the server should *not* disconnect a client after a BOOM message. */
    private final boolean debug;
    /** Board for managing the game */
    private final Board board;
    /** Number of players */
    private int players;

    // Abstraction function
    //    represent the multiplayer server of the game with a port, a board and a debug option
    // Rep invariant
    //    0 <= port <= 65535
    // Rep exposure
    ...
    private void checkRep() {
        assert this.serverSocket.getLocalPort() >= 0;
        assert this.serverSocket.getLocalPort() <= MAXIMUM_PORT;
     ...
     */
    public synchronized void increasePlayer(){
        this.players += 1;
    }
    
    /**
     * Modifier
     * Decrease the number of player
     * @param number of players
     */
    public synchronized void decreasePlayer(){
        this.players -= 1;
    }
    ...
    private void handleConnection(Socket socket) throws IOException {
        ...
        try {
            this.increasePlayer();
            out.println("Welcome to Minesweeper. Players: " + this.players +
                    " including you. Board: " + this.board.getCol() +" columns by " +
                    this.board.getRow() + " rows. Type 'help' for help.");
            for (String line = in.readLine(); line != null; line = in.readLine()) {
                ...
                if (output.startsWith("BOOM!") && !debug) {
                    break;
                }
            }
        } finally {
    ...
    public static void runMinesweeperServer(boolean debug, Optional<File> file, int sizeX, int sizeY, int port) throws IOException {
        Board jeu;
        if (file.isPresent()) {
            jeu = new Board(file.get());
        }
        else{
            jeu = new Board(sizeX, sizeY);
        }
        MinesweeperServer server = new MinesweeperServer(port, debug, jeu);
        server.serve();
    }
}
