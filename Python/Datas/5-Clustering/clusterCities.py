...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        set1 = self.points
        set2 = other.points
        minDist = set1[0].distance(set2[0])
        for point in set1:
            for otherPoint in set2:
                testDist = point.distance(otherPoint)
                if testDist < minDist:
                    minDist = testDist
        return minDist
    ...
    def maxLinkageDist(self, other):
        ...
        set1 = self.points
        set2 = other.points
        maxDist = set1[0].distance(set2[0])
        for point in set1:
            for otherPoint in set2:
                testDist = point.distance(otherPoint)
                if testDist > maxDist:
    ...
    def averageLinkageDist(self, other):
        ...
        for point in set1:
            for otherPoint in set2:
                somme += point.distance(otherPoint)
                counter += 1
        return somme / float(counter)
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        for p2 in c2.members():
            if p2 not in newPoints:
                newPoints.append(p2)
        self.members.remove(c1)
        self.members.remove(c2)
        self.add(Cluster(newPoints, p2.getAttrs()))

    def findClosest(self, linkage):
        """ Returns a tuple containing the two most similar
        clusters in self
        Closest defined using the metric linkage """
        clust1 = self.getClusters()[0]
        clust2 = self.getClusters()[1]
        minDist = linkage(clust1, clust2)
        res = (clust1, clust2)
        for firstCluster in self.getClusters():
            for secondCluster in self.getClusters():
                if firstCluster != secondCluster:
                    testDist = linkage(firstCluster, secondCluster)
                    if testDist < minDist:
                        minDist = testDist
                        res = (firstCluster, secondCluster)
        return res
    ...
    def mergeOne(self, linkage):
        ...
        candidate = self.findClosest(linkage)
        self.mergeClusters(candidate[0], candidate[1])
        return candidate
...
def readCityData(fName, scale=False):
    ...
    for line in dataFile:
        line = line[:-1]
        dataLine = line.split(sep=',')  # remove newline; then split
