...
class Patient(object):
    ...
    def update(self):
        ...
        for j in range(self.getTotalPop()):
            try:
                new_virus = self.viruses[j].reproduce(popDensity)
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for l in range(numTrials):
        ...
        for m in range(300):
            patient.update()
            results[m].append(patient.getTotalPop())
    for i in range(300):
        results[i] = sum(results[i]) / float(numTrials)
    pylab.plot(results)
...
class ResistantVirus(SimpleVirus):
    ...
    def __init__(self, maxBirthProb, clearProb, resistances, mutProb):
        ...
        of this virus particle's resistance
        (either True or False) to each drug.
        e.g. {'guttagonol':False, 'srinol':False}, means that this virus
        particle is resistant to neither guttagonol nor srinol.

        mutProb: Mutation probability for this virus particle (a float).
        This is the probability of the offspring acquiring
        or losing resistance to a drug.
        """
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            if self.isResistantTo(drug) is False:
                reprod = False
        if reprod is True:
            if random.random() <= self.maxBirthProb * (1 - popDensity):
                Resistances = {}
                for key in self.resistances:
                    if random.random() <= self.mutProb:
                        Resistances[key] = not(self.resistances[key])
                    else:
                        Resistances[key] = (self.resistances[key])
                return ResistantVirus(self.maxBirthProb,
                                      self.clearProb, Resistances,
                                      self.mutProb)
            else:
                raise NoChildException()
        else:
...
class TreatedPatient(Patient):
    ...
    def addPrescription(self, newDrug):
        ...
        postcondition: The list of drugs being administered to
        a patient is updated
        """
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    resultstot = [[] for k in range(300)]
    resultsres = [[] for k in range(300)]
    for l in range(numTrials):
        viruses = []
        for k in range(numViruses):
            viruses.append(ResistantVirus(maxBirthProb,
        ...
        for m in range(300):
            ...
            resultsres[m].append(patient.getResistPop(["guttagonol"]))
    for i in range(300):
